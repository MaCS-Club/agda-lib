...
_[_≡_] : ∀ {α} (A : I → Set α) → A i0 → A i1 → Set α
_[_≡_] = PathP
...
face0 : ∀ {α} {A : Set α} {a b : A} → (p : Path A a b) → A
face0 p = p i0

face1 : ∀ {α} {A : Set α} {a b : A} → (p : Path A a b) → A
face1 p = p i1

funExt : ∀ {α β} {A : Set α} {B : Set β} {f g : A → B} → (p : (x : A) → f x ≡ g x) → f ≡ g
funExt p = λ i → (λ a → (p a) i)
